...
export default function TableStatusGridScreen() {
  ...
  const { setIsFullView } = useFullView();
  const lastScrollY = useRef(0);
  const horizontalScrollRef = useRef<ScrollView>(null);
  const stickyHeaderScrollRef = useRef<ScrollView>(null);
  const rotatedHeaderScrollRef = useRef<ScrollView>(null);
  const rotatedContentScrollRef = useRef<ScrollView>(null);
  const rotatedVerticalScrollRef = useRef<ScrollView>(null);
  ...
  const handleRotatedHorizontalScroll = (event: any) => {
    const offsetX = event.nativeEvent.contentOffset.x;
    // Sync header scroll with content scroll
    if (rotatedHeaderScrollRef.current) {
      rotatedHeaderScrollRef.current.scrollTo({ x: offsetX, animated: false });
    }
  };

  const handleRotatedHeaderScroll = (event: any) => {
    const offsetX = event.nativeEvent.contentOffset.x;
    // Sync content scroll with header scroll
    if (rotatedContentScrollRef.current) {
      rotatedContentScrollRef.current.scrollTo({ x: offsetX, animated: false });
    }
  };
  ...
  const loadBranches = async () => {
    try {
      ...
      // Keep selection stable/valid
      if (loadedBranches.length === 1 && loadedBranches[0]?.id && !selectedBranchId) {
        setSelectedBranchId(loadedBranches[0].id);
        return;
      }

      if (selectedBranchId && !loadedBranches.some((b) => b.id === selectedBranchId)) {
        setSelectedBranchId(loadedBranches[0]?.id || "");
      }
    } catch (error: any) {
  ...
  const loadGridData = async () => {
    ...
    try {
      ...
      setToast({
        visible: true,
        message: error.message || t("admin.tableStatusGrid.errorLoading"),
        type: "error",
      });
    } finally {
      setLoading(false);
  ...
  // Sync isRotated with FullViewContext
  useEffect(() => {
    setIsFullView(isRotated);
  }, [isRotated, setIsFullView]);
  ...
  return (
    <View style={styles.container}>
      ...
      <ScrollView
        ...
        contentContainerStyle={{ paddingTop: headerHeight + 16, ...styles.scrollContent }}
        refreshControl={
          <RefreshControl
      ...
      >
        ...
        {/* Full View Button */}
        {!loading && gridData && gridData.tables.length > 0 && (
          <View style={styles.fullViewButtonContainer}>
            <TouchableOpacity
              style={styles.fullViewButton}
              onPress={() => setIsRotated(!isRotated)}
            >
              <MaterialCommunityIcons 
                name={isRotated ? "arrow-expand" : "arrow-collapse"} 
                size={18} 
                color="#ec4899" 
              />
              <Text style={styles.fullViewButtonText}>
                {isRotated ? t("admin.tableStatusGrid.normal") : t("admin.tableStatusGrid.fullView")}
              </Text>
            </TouchableOpacity>
          </View>
        )}
        ...
        {!selectedBranchId ? (
          ...
          <View style={[
            ...
            {
              width: SCREEN_HEIGHT - insets.top,
              height: SCREEN_WIDTH,
              transform: [{ rotate: '90deg' }],
              left: SCREEN_WIDTH / 2 - (SCREEN_HEIGHT - insets.top) / 2,
              top: SCREEN_HEIGHT / 2 - SCREEN_WIDTH / 2 + insets.top - 20,
            }
          ]}>
            {/* Full View Controls - Top */}
            {showDatePicker && (
              <View style={styles.fullViewDateControls}>
                <TouchableOpacity
                  style={styles.fullViewControlButton}
                  onPress={() => handleDateChange(-1)}
                >
                  <MaterialCommunityIcons name="chevron-left" size={18} color="#ec4899" />
                </TouchableOpacity>
                <TouchableOpacity
                  style={styles.fullViewDateButton}
                  onPress={() => setDatePickerVisible(true)}
                >
                  <Text style={styles.fullViewDateText}>
                ...
                <TouchableOpacity
                  ...
                  onPress={() => handleDateChange(1)}
                >
                  <MaterialCommunityIcons name="chevron-right" size={18} color="#ec4899" />
                </TouchableOpacity>
                <TouchableOpacity
                  style={styles.fullViewControlButton}
                  onPress={() => setSelectedDate(new Date())}
                >
                  <Text style={styles.fullViewControlText}>
            ...
            <TouchableOpacity
              style={styles.fullViewToggleButton}
            ...
            {/* Sticky Header Row */}
            <View style={styles.rotatedStickyHeader}>
              {/* Fixed Table Name Header */}
              <View style={styles.rotatedTableHeaderCell}>
                <Text style={styles.tableHeaderText}>Table</Text>
              </View>
              {/* Horizontal Scroll for Time Slot Headers */}
              <ScrollView
                ref={rotatedHeaderScrollRef}
                horizontal
                showsHorizontalScrollIndicator={false}
                style={styles.rotatedHeaderScroll}
                contentContainerStyle={styles.rotatedHeaderScrollContent}
                nestedScrollEnabled={true}
                bounces={false}
                onScroll={handleRotatedHeaderScroll}
                scrollEventThrottle={16}
              >
                {filteredTimeSlots.map((timeSlot) => (
                  <View key={timeSlot} style={styles.rotatedTimeSlotHeaderCell}>
                    <Text style={styles.timeSlotHeaderText}>{timeSlot}</Text>
                  </View>
                ))}
              </ScrollView>
            ...
            {/* Scrollable Content */}
            <ScrollView
              ref={rotatedVerticalScrollRef}
              style={[
                styles.rotatedGridScroll,
                {
                  width: SCREEN_HEIGHT - insets.top,
                  height: SCREEN_WIDTH - 60, // Subtract header height
                }
              ]}
              contentContainerStyle={styles.rotatedGridScrollContent}
              showsVerticalScrollIndicator={true}
              bounces={false}
              nestedScrollEnabled={true}
            >
              <View style={styles.rotatedGridWrapper}>
                {/* Fixed Table Name Column */}
                <View style={styles.rotatedFixedColumn}>
                  {paginatedTables.map((table, index) => (
                    <View 
                      key={table.id} 
                      style={[
                        styles.rotatedTableCellSticky,
                        index % 2 === 1 && styles.rotatedTableCellStickyAlternate
                      ]}
                    >
                ...
                <ScrollView
                  ref={rotatedContentScrollRef}
                  horizontal
                  showsHorizontalScrollIndicator={true}
                  style={styles.rotatedHorizontalScroll}
                  contentContainerStyle={styles.rotatedHorizontalScrollContent}
                  nestedScrollEnabled={true}
                  bounces={false}
                  onScroll={handleRotatedHorizontalScroll}
                  scrollEventThrottle={16}
                >
                  <View style={styles.rotatedGrid}>
                    {paginatedTables.map((table, index) => (
                      <View 
                        key={table.id} 
                        style={[
                          styles.rotatedGridRow,
                          index % 2 === 1 && styles.rotatedGridRowAlternate
                        ]}
                      >
                        {filteredTimeSlots.map((timeSlot) => {
            ...
            {/* Full View Controls - Bottom */}
            <View style={styles.fullViewBottomControls}>
              <TouchableOpacity
                style={styles.fullViewExitButton}
                onPress={() => setIsRotated(false)}
              >
                <MaterialCommunityIcons name="close" size={18} color="#fff" />
                <Text style={styles.fullViewExitButtonText}>
                  {t("admin.tableStatusGrid.exitFullView")}
                </Text>
              </TouchableOpacity>
              {!hasFilters && filteredTables.length > ITEMS_PER_PAGE && (
                <View style={styles.fullViewPagination}>
                  <TouchableOpacity
                    style={[styles.fullViewControlButton, currentPage === 1 && styles.fullViewControlButtonDisabled]}
                    onPress={() => setCurrentPage((prev) => Math.max(1, prev - 1))}
                    disabled={currentPage === 1}
                  >
                    <MaterialCommunityIcons name="chevron-left" size={18} color={currentPage === 1 ? "#6B7280" : "#ec4899"} />
                  </TouchableOpacity>
                  <Text style={styles.fullViewPaginationText}>
                    {currentPage} / {totalPages}
                  </Text>
                  <TouchableOpacity
                    style={[styles.fullViewControlButton, currentPage === totalPages && styles.fullViewControlButtonDisabled]}
        ...
        >
          <Pressable style={styles.bottomSheetContent} onPress={(e) => e.stopPropagation()}>
            ...
            <View style={styles.dropdownHeader}>
              <Text style={styles.dropdownTitle}>{t("admin.tableStatusGrid.selectBranch") || "Select Branch"}</Text>
              <TouchableOpacity onPress={() => setBranchDropdownVisible(false)}>
            ...
            <ScrollView style={styles.dropdownList} showsVerticalScrollIndicator={true}>
              {branches.map((branch) => (
                <TouchableOpacity
                  ...
                  style={[
                    styles.dropdownItem,
                    selectedBranchId === branch.id && styles.dropdownItemActive,
                  ]}
                  onPress={() => {
                ...
                >
                  <Text
                    style={[
                      ...
                      selectedBranchId === branch.id && styles.dropdownItemTextActive,
                    ]}
                  >
      ...
      >
        ...
        >
          <Pressable style={styles.bottomSheetContent} onPress={(e) => e.stopPropagation()}>
            ...
            <View style={styles.dropdownHeader}>
              <Text style={styles.dropdownTitle}>{t("admin.tableStatusGrid.selectZone")}</Text>
              <TouchableOpacity onPress={() => setZoneDropdownVisible(false)}>
                <MaterialCommunityIcons name="close-circle" size={24} color="#9CA3AF" />
              </TouchableOpacity>
            </View>
            <ScrollView style={styles.dropdownList} showsVerticalScrollIndicator={true}>
              <TouchableOpacity
                style={[
                  styles.dropdownItem,
                  selectedZone === "all" && styles.dropdownItemActive,
                ]}
                onPress={() => {
                  setSelectedZone("all");
              ...
              >
                <Text
                  style={[
                    styles.dropdownItemText,
                    selectedZone === "all" && styles.dropdownItemTextActive,
                  ]}
                >
                  {t("admin.tableStatusGrid.allZones")}
                </Text>
              ...
              <TouchableOpacity
                style={[
                  styles.dropdownItem,
                  selectedZone === "no-zone" && styles.dropdownItemActive,
                ]}
                onPress={() => {
              ...
              >
                <Text
                  style={[
                    styles.dropdownItemText,
                    selectedZone === "no-zone" && styles.dropdownItemTextActive,
                  ]}
                >
                  {t("admin.tableStatusGrid.unassigned")}
                </Text>
      ...
      >
        ...
        >
          <Pressable style={styles.modalContent} onPress={(e) => e.stopPropagation()}>
            <View style={styles.modalHeader}>
              <Text style={styles.modalTitle}>{t("admin.tableStatusGrid.selectDate")}</Text>
