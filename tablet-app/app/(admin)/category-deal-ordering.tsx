...
export default function CategoryDealOrderingScreen() {
  ...
  const canReorderCategoryDeals =
    !permissionsLoading && canAny([{ resource: RESOURCES.DEALS, action: ACTIONS.REORDER_CATEGORY }]);
  ...
  const loadCategoryDeals = useCallback(async () => {
    ...
    try {
      ...
      setToast({
        visible: true,
        message: t("admin.categoryDealOrdering.loadError", { defaultValue: "Failed to load deals" }),
        type: "error",
      });
    } finally {
      setLoading(false);
  ...
  const hasChanges = useMemo(() => !arraysEqual(deals.map((item) => item.id), initialOrder), [deals, initialOrder]);
  ...
  const handleReset = useCallback(() => {
    const sortedDeals = [...deals].sort((a, b) => {
      const orderA = typeof a.deal.listOrder === "number" && a.deal.listOrder > 0 ? a.deal.listOrder : Number.MAX_SAFE_INTEGER;
      const orderB = typeof b.deal.listOrder === "number" && b.deal.listOrder > 0 ? b.deal.listOrder : Number.MAX_SAFE_INTEGER;
  ...
  const handleSave = useCallback(async () => {
    ...
    try {
      ...
      setToast({
        visible: true,
        message: t("admin.categoryDealOrdering.saveSuccess", { defaultValue: "Deal order updated" }),
        type: "success",
      });
      ...
      setToast({
        visible: true,
        message: t("admin.categoryDealOrdering.saveError", { defaultValue: "Failed to save deal order" }),
        type: "error",
      });
    } finally {
      setSaving(false);
  ...
  const renderDealItem = ({ item, drag, isActive, getIndex }: RenderItemParams<DealItem>) => {
  ...
  if (loading && !category) {
    return (
      <View key={languageKey} style={styles.container}>
        <AnimatedHeader title={t("admin.categoryDealOrdering.title", { defaultValue: "Deal Ordering" })} onBackPress={() => router.back()} />
        <View style={styles.loadingContainer}>
          <ActivityIndicator size="large" color="#ec4899" />
          <Text style={styles.loadingText}>{t("admin.menuManagement.loading", { defaultValue: "Loading..." })}</Text>
        </View>
      </View>
    );
  }
    ...
    return (
      <View key={languageKey} style={styles.container}>
        ...
        <View style={styles.emptyContainer}>
          <Text style={styles.emptyText}>{t("admin.categoryDealOrdering.categoryNotFound", { defaultValue: "Category not found" })}</Text>
        </View>
      </View>
    );
  }
        ...
        {loading && deals.length === 0 ? (
          ...
          <View style={styles.content}>
            <View style={styles.section}>
              ...
              {deals.length === 0 ? (
                <View style={styles.emptyContainer}>
                  <Text style={styles.emptyText}>{t("admin.categoryDealOrdering.emptyTitle", { defaultValue: "No deals found" })}</Text>
                </View>
              ) : (
                <DraggableFlatList
                  data={deals}
              ...
              <View style={styles.sectionActions}>
                <TouchableOpacity style={[styles.resetButton, (!hasChanges || saving) && styles.resetButtonDisabled]} onPress={handleReset} disabled={!hasChanges || saving}>
                  <Text style={[styles.resetButtonText, (!hasChanges || saving) && styles.resetButtonTextDisabled]}>
                ...
                >
                  {saving ? <ActivityIndicator size="small" color="#fff" /> : <MaterialCommunityIcons name="content-save" size={16} color="#fff" />}
                  <Text style={styles.saveButtonText}>
                    {saving ? t("admin.categoryDealOrdering.saving", { defaultValue: "Saving..." }) : t("admin.categoryDealOrdering.save", { defaultValue: "Save" })}
                  </Text>
                </TouchableOpacity>
              </View>
            </View>
