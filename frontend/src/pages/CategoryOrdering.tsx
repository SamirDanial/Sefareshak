...
import { mdiArrowLeft, mdiDragVertical, mdiRefresh, mdiStar, mdiFormatListNumbered } from "@mdi/js";
...
const CategoryOrdering: React.FC = () => {
  ...
  const loadOrderingCategories = useCallback(async () => {
    try {
      ...
      toast.error(
        t("admin.categoryManagement.ordering.loadError", {
          defaultValue: "Failed to load categories. Please try again.",
        })
      );
    } finally {
  ...
  const handleSaveOrder = useCallback(
    ...
    [featuredOrder, listOrder, getToken, loadOrderingCategories, t]
  );
      ...
      <div className="flex flex-col sm:flex-row gap-4 items-start sm:items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            {t("admin.categoryManagement.ordering.description")}
          </p>
        </div>
        <div className="flex flex-wrap gap-2">
          ...
          >
            <Icon path={mdiRefresh} size={0.67} className={cn(
                "mr-2 h-4 w-4",
                orderingLoading && "animate-spin"
              )} />
            {t("admin.categoryManagement.ordering.refresh", {
        ...
        <CardHeader>
          ...
          <p className="text-sm text-muted-foreground">
            {t("admin.categoryManagement.ordering.helper", {
              defaultValue:
                "Use the drag handles to change how categories appear to customers.",
            })}
          </p>
        </CardHeader>
        <CardContent>
            ...
            <div className="space-y-4 rounded-2xl border border-border/70 bg-muted/20 p-4">
              <div className="flex items-start justify-between gap-3">
                ...
                <span className="text-xs font-medium text-muted-foreground">
                  {t("admin.categoryManagement.ordering.featuredCount", {
                    ...
                    <div className="space-y-2">
                      {featuredItems.map((category, index) => (
                        <SortableCategoryItem
                          ...
                          index={index}
                          helperText={t(
                            "admin.categoryManagement.ordering.featuredHelper",
                            { position: index + 1 }
                          )}
                        />
                      ))}
                    ...
                    <div className="space-y-2">
                      {listItems.map((category, index) => (
                        <SortableCategoryItem
                          ...
                          index={index}
                          helperText={t(
                            "admin.categoryManagement.ordering.listHelper",
                            { position: index + 1 }
                          )}
                        />
                      ))}
