...
const BusinessDayClosedDays: React.FC = () => {
  ...
  const refresh = async () => {
    ...
    try {
      ...
      toast.error(
        e?.message ||
          t("admin.businessDayClosedDays.errors.load", {
            defaultValue: "Failed to load closed days",
          })
      );
    } finally {
      setIsLoading(false);
  ...
  if (!canViewReports) {
    return (
      <Card>
        <CardHeader>
          <CardTitle>{t("admin.businessDayClosedDays.title", { defaultValue: "Closed Days" })}</CardTitle>
        </CardHeader>
        <CardContent>
  ...
  return (
    <div className="space-y-6 p-6">
      <PageHeader
        ...
        description={t("admin.businessDayClosedDays.description", {
          defaultValue: "Browse previously closed sessions and view their details.",
        })}
        actions={
          <Button
            variant="outline"
      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-base">
            {t("admin.businessDayClosedDays.filterTitle", { defaultValue: "Filter" })}
          </CardTitle>
        </CardHeader>
        <CardContent className="space-y-4">
          <div className="flex flex-col gap-2">
            <Label className="text-sm">
              {t("admin.businessDayClosedDays.branchLabel", { defaultValue: "Branch" })}
            </Label>
            <Select
            ...
            >
              <SelectTrigger className="bg-transparent text-foreground border-border w-full sm:w-auto sm:min-w-[220px]">
                <SelectValue
                  placeholder={t("admin.businessDayClosedDays.selectBranchPlaceholder", {
      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-base">
            {t("admin.businessDayClosedDays.listTitle", { defaultValue: "Closed days" })}
          </CardTitle>
        </CardHeader>
        <CardContent>
          {isLoading ? (
            ...
            <div className="text-sm text-muted-foreground">
              {t("admin.businessDayClosedDays.selectBranchToView", {
                defaultValue: "Select a branch to view closed days.",
              })}
            </div>
          ) : sessions.length === 0 ? (
            <div className="text-sm text-muted-foreground">
              {t("admin.businessDayClosedDays.empty", { defaultValue: "No closed days found." })}
            </div>
          ) : (
                    ...
                    <div className="flex items-center justify-between gap-3">
                      <div>
                        <div className="text-sm font-semibold">
                          {t("admin.businessDayClosedDays.sessionLabel", {
                            number: s.sequenceNumber,
                            defaultValue: `Session #${s.sequenceNumber}`,
                          })}
                        </div>
