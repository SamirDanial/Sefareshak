...
import { mdiArrowLeft, mdiReceipt, mdiRefresh } from "@mdi/js";
...
const BusinessDayClosedDayDetails: React.FC = () => {
  ...
  const rawReportJson = useMemo(() => {
    ...
    try {
      ...
      return String((report as any).data || "");
    }
  }, [report]);
  ...
  const refresh = async () => {
    ...
    try {
      ...
      toast.error(
        e?.message ||
          t("admin.businessDayClosedDayDetails.errors.loadReport", {
            defaultValue: "Failed to load report",
          })
      );
    } finally {
      setIsLoading(false);
  ...
  const formatDateDMonYYYY = (value: string | Date | undefined | null) => {
    if (!value) return "—";
  ...
  const buildOrderVatGroups = (items: any[]) => {
    ...
    for (const it of items || []) {
      ...
      for (const a of it?.orderItemAddOns || []) {
        const addonRate =
          a?.taxPercentage !== undefined && a?.taxPercentage !== null
            ? toNum(a?.taxPercentage)
            : baseRateForAddons;
        ...
        map.set(addonRate, [
          ...(map.get(addonRate) || []),
          { key: `${it?.id || "item"}:${a?.id || addonLabel}`, label: addonLabel, amount: addonTotal },
        ]);
      }
    }
        ...
        <CardHeader>
          <CardTitle>
            {t("admin.businessDayClosedDayDetails.title", {
              defaultValue: "Closed Day Details",
            })}
          </CardTitle>
        </CardHeader>
        <CardContent>
      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-base">
            {t("admin.businessDayClosedDayDetails.sessionTitle", { defaultValue: "Session" })}
          </CardTitle>
        </CardHeader>
        <CardContent>
          {!sessionMeta ? (
            <div className="text-sm text-muted-foreground">
              {t("admin.businessDayClosedDayDetails.loadingSession", {
                defaultValue: "Loading session details...",
              })}
            </div>
          ) : (
            <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
              <div className="rounded-lg border border-border p-3">
                <div className="text-xs text-muted-foreground">
                  {t("admin.businessDayClosedDayDetails.sessionLabel", { defaultValue: "Session" })}
                </div>
                <div className="text-sm font-semibold">#{(sessionMeta as any)?.sequenceNumber}</div>
              </div>
              <div className="rounded-lg border border-border p-3">
                <div className="text-xs text-muted-foreground">
                  {t("admin.businessDayClosedDayDetails.startedLabel", { defaultValue: "Started" })}
                </div>
                <div className="text-sm font-semibold">{formatDateDMonYYYY((sessionMeta as any)?.startedAt)}</div>
              </div>
              <div className="rounded-lg border border-border p-3">
                <div className="text-xs text-muted-foreground">
                  {t("admin.businessDayClosedDayDetails.endedLabel", { defaultValue: "Ended" })}
                </div>
                <div className="text-sm font-semibold">{formatDateDMonYYYY((sessionMeta as any)?.endedAt)}</div>
              </div>
            </div>
          )}
        </CardContent>
      ...
      {report ? (
        <Card>
          <CardHeader>
            <div className="flex items-center justify-between gap-3">
              <CardTitle className="text-base">
                {t("admin.businessDayClosedDayDetails.dailyReportTitle", {
            ...
            {isLoading ? (
              ...
              <div className="space-y-3">
                <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
                  <div className="rounded-lg border border-border p-3">
                    <div className="text-xs text-muted-foreground">
                      {t("admin.businessDayClosedDayDetails.grossSales", { defaultValue: "Gross sales" })}
                    </div>
                  ...
                  <div className="rounded-lg border border-border p-3">
                    <div className="text-xs text-muted-foreground">
                      {t("zReport.drinks", { defaultValue: "Drinks" })}
                    </div>
                    <div className="text-sm font-semibold">
                      {(report as any)?.data?.totals?.drinksGross?.toFixed?.(2) ??
                        (report as any)?.data?.totals?.drinksGross ??
                        "0.00"}
                    </div>
                  </div>
                  <div className="rounded-lg border border-border p-3">
                    <div className="text-xs text-muted-foreground">
                      {t("admin.businessDayClosedDayDetails.taxTotal", { defaultValue: "Tax total" })}
                    </div>
                    <div className="text-sm font-semibold">
        ...
        <Card>
          <CardHeader>
            <CardTitle className="text-base">
              {t("admin.businessDayClosedDayDetails.dailyReportTitle", { defaultValue: "Daily report" })}
            </CardTitle>
          </CardHeader>
          <CardContent>
            <div className="text-sm text-muted-foreground">
              {t("admin.businessDayClosedDayDetails.noReportLoaded", {
                defaultValue: "No report loaded.",
              })}
            </div>
          </CardContent>
        </Card>
              ...
              <div className="px-3 py-3 font-mono text-[11px] leading-4">
                <>
                  <div className="text-center font-semibold">
                    ...
                    {(sessionMeta as any)?.sequenceNumber ?? "—"}
                  </div>
                  <div className="text-center">
                    {formatDateDMonYYYY((sessionMeta as any)?.startedAt)} {t("zReport.to", { defaultValue: "-" })}{" "}
                    {formatDateDMonYYYY((sessionMeta as any)?.endedAt)}
                  </div>
                  <div className="text-center">
                    {t("zReport.performedAt", { defaultValue: "Performed at" })} {formatDateDMonYYYY((sessionMeta as any)?.endedAt)}
                  </div>
                  {headerTitle ? <div className="mt-1 text-center font-semibold">{headerTitle}</div> : null}
                  ...
                  <div className="text-center text-black/70">{t("zReport.taxLine", { defaultValue: "—" })}</div>

                  <div className="my-2 border-t border-dashed border-black/40" />

                  <div className="flex justify-between">
                    <span>{t("zReport.totalOrders", { defaultValue: "Orders" })}</span>
                    <span>{String((zReport as any)?.counts?.totalOrders ?? (report as any)?.data?.counts?.totalOrders ?? "—")}</span>
                  </div>
                  <div className="flex justify-between">
                    <span>{t("zReport.cancelledOrders", { defaultValue: "Cancelled" })}</span>
                    <span>{String((zReport as any)?.counts?.cancelledOrders ?? "—")}</span>
                  </div>

                  <div className="flex justify-between">
                    <span>{t("zReport.articles", { defaultValue: "Artikel" })}</span>
                    <span>{formatMaybeMoney((zReport as any)?.sales?.lines?.articlesGross)}</span>
                  </div>

                  {(zReport as any)?.sales?.lines?.foodsGross === null ||
                  (zReport as any)?.sales?.lines?.foodsGross === undefined ? null : (
                    <div className="flex justify-between">
                      <span>{t("zReport.foods", { defaultValue: "Foods" })}</span>
                      <span>{formatMaybeMoney((zReport as any)?.sales?.lines?.foodsGross)}</span>
                    </div>
                  )}

                  <div className="flex justify-between">
                    <span>{t("zReport.discount", { defaultValue: "Rabatt" })}</span>
                    <span>{formatMaybeMoney(Number((zReport as any)?.sales?.lines?.discountGross || 0))}</span>
                  </div>
                  <div className="flex justify-between">
                    <span>{t("zReport.drinks", { defaultValue: "Getränke" })}</span>
                    <span>{formatMaybeMoney((zReport as any)?.sales?.lines?.drinksGross)}</span>
                  </div>
                  <div className="flex justify-between">
                    <span>{t("zReport.deliveryFee", { defaultValue: "Anfahrt" })}</span>
                    <span>{formatMaybeMoney((zReport as any)?.sales?.lines?.deliveryFeeGross)}</span>
                  </div>

                  {(() => {
                    const takeaway = Number((zReport as any)?.sales?.lines?.takeawayServiceFeeGross || 0);
                  ...
                  <div className="flex justify-between">
                    ...
                    <span>{formatMaybeMoney((zReport as any)?.sales?.lines?.distanceKm ?? 0)}</span>
                  </div>
                  ...
                  <div className="flex justify-between">
                    ...
                    <span>
                      {formatMaybeMoney(
                        (zReport as any)?.sales?.sums?.sumCancelledGross ??
                          -((zReport as any)?.sales?.sums?.refundedGross ?? (report as any)?.data?.totals?.refunded ?? 0)
                      )}
                    </span>
                  </div>
                  ...
                  <div className="flex justify-between font-semibold">
                    ...
                    <span>
                      {formatMaybeMoney(
                        (zReport as any)?.sales?.sums?.totalRevenueGross ??
                          (report as any)?.data?.totals?.netSales ??
                          (report as any)?.data?.totals?.grossSales
                      )}
                    </span>
                  </div>
                    ...
                    {(
                      ...
                      return (
                        <React.Fragment key={g.key}>
                          ...
                          <div className="flex justify-between font-semibold">
                            <span>
                              {t("zReport.totalVatByGroup", { defaultValue: "Gesamt" })} {g.title}{" "}
                              {t("zReport.vatRate", { defaultValue: "MwSt" })}:
                            </span>
