  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (amount && parseFloat(amount) > 0) {
        validationData.amount = parseFloat(amount);
        validationData.currency = currency;
        validationData.paymentMethod = paymentMethod.toUpperCase() as "CASH" | "ONLINE";
        validationData.paymentStatus = paymentStatus.toUpperCase() as "PENDING" | "PAID" | "FAILED";
      }
      ...
      if (mode === "edit") {
        ...
        const result = await branchService.updateValidation(organization.id, existingValidation.id, validationData, token);
        showToast(t("admin.organizations.validation.validationDialog.validationUpdated"), "success");
      } else {
        const result = await branchService.createValidation(organization.id, validationData, token);
                ...
                {!organization?.freeVersion && (
                  <>
                    ...
                    {amount && parseFloat(amount) > 0 && (
                      <View style={styles.field}>
                        <Text style={styles.label}>
                          {t("admin.organizations.validation.validationDialog.currency")}
                        </Text>
                        <TouchableOpacity
                          style={styles.selectButton}
                          onPress={() => {
                        ...
                        >
                          <Text style={styles.selectText}>{t(`admin.organizations.validation.validationDialog.${currency.toLowerCase()}`)}</Text>
