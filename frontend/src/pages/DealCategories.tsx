...
import { useDealCategories } from "@/hooks/useApi";
      ...
      <div>
        {loading ? (
          ...
          <div className="text-center py-8">
            <p className="text-red-500">Failed to load deal categories: {error}</p>
          </div>
        ) : categories.length === 0 ? (
          <div className="text-center py-12">
            <p className="text-muted-foreground">{t("home.noDealCategories")}</p>
          </div>
        ) : (
          <div className="flex flex-wrap" style={{ justifyContent: "space-between", paddingTop: 12, paddingBottom: 12 }}>
            {categories.map((category, index) => (
              <div
                ...
                onClick={() => navigate(`/deal-category/${category.id}`)}
              >
                <div className="relative w-full overflow-hidden" style={{ height: 200, backgroundColor: "#262626" }}>
                  <img
                    src={
                      category.image
                  ...
                  />
                  <div className="absolute inset-0" style={{ backgroundColor: "rgba(0, 0, 0, 0)" }} />
                </div>
