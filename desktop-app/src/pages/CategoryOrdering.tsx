...
import { ChevronLeft, GripVertical, RefreshCw, Star, ListOrdered } from "lucide-react";
...
const CategoryOrdering: React.FC = () => {
  ...
  const categoryDictionary = useMemo(() => {
    ...
    (orderingCategories || []).forEach((category) => {
  ...
  const sortByOrder = useCallback((data: Category[], field: "featuredOrder" | "listOrder") => {
    return [...data].sort((a, b) => {
      const orderA =
        typeof (a as any)[field] === "number" && (a as any)[field] > 0
          ? (a as any)[field]
          : Number.MAX_SAFE_INTEGER;
      const orderB =
        typeof (b as any)[field] === "number" && (b as any)[field] > 0
          ? (b as any)[field]
          : Number.MAX_SAFE_INTEGER;
  ...
  const loadOrderingCategories = useCallback(async () => {
    try {
      ...
      alert(t("admin.categoryManagement.ordering.loadError"));
    } finally {
  ...
  const handleSaveOrder = useCallback(
    async (type: "featured" | "list") => {
      try {
        ...
        alert(
          t(
            `admin.categoryManagement.ordering.${
    ...
    <div style={{ padding: "24px", height: "100%", overflow: "auto" }}>
      <div style={{ marginBottom: "18px" }}>
        <PageHeader
          ...
          description={t("admin.categoryManagement.ordering.description")}
          actions={
            <>
              <button
                type="button"
      ...
      >
        <div style={{ fontSize: "14px", fontWeight: 900, color: "#ec4899", marginBottom: "8px" }}>
          {t("admin.categoryManagement.ordering.sectionsTitle")}
        </div>
        <div style={{ fontSize: "13px", color: "#6b7280", marginBottom: "16px" }}>
          {t("admin.categoryManagement.ordering.helper")}
        </div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "16px" }}>
            ...
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: "10px" }}>
              <div>
                ...
                <div style={{ fontSize: "12px", color: "#6b7280", marginTop: "4px" }}>
                  {t("admin.categoryManagement.ordering.featuredDescription")}
                </div>
              </div>
              <div style={{ fontSize: "12px", color: "#6b7280", fontWeight: 900 }}>
                {t("admin.categoryManagement.ordering.featuredCount", { count: featuredItems.length })}
              </div>
            </div>
                    ...
                    <div style={{ display: "flex", flexDirection: "column", gap: "10px" }}>
                      {featuredItems.map((category, index) => (
                        <SortableCategoryItem
                          ...
                          category={category}
                          index={index}
                          helperText={t("admin.categoryManagement.ordering.featuredHelper", {
                            position: index + 1,
                          })}
                        />
                      ))}
          ...
          <div style={{ border: "1px solid #e5e7eb", borderRadius: "16px", padding: "14px", backgroundColor: "#f9fafb" }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: "10px" }}>
              <div>
                <div style={{ display: "flex", alignItems: "center", gap: "8px", fontWeight: 900, color: "#111827" }}>
                  <ListOrdered style={{ width: 16, height: 16, color: "#ec4899" }} />
                  {t("admin.categoryManagement.ordering.listTitle")}
                </div>
                <div style={{ fontSize: "12px", color: "#6b7280", marginTop: "4px" }}>
                  {t("admin.categoryManagement.ordering.listDescription")}
                </div>
              </div>
              <div style={{ fontSize: "12px", color: "#6b7280", fontWeight: 900 }}>
                {t("admin.categoryManagement.ordering.listCount", { count: listItems.length })}
              </div>
            </div>
                    ...
                    <div style={{ display: "flex", flexDirection: "column", gap: "10px" }}>
                      {listItems.map((category, index) => (
                        <SortableCategoryItem
                          ...
                          category={category}
                          index={index}
                          helperText={t("admin.categoryManagement.ordering.listHelper", {
                            position: index + 1,
                          })}
                        />
                      ))}
