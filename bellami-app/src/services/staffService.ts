...
export const staffService = {
  ...
  getUserBranches: async (
    ...
    const response = await apiService.get(
      `/api/staff/${userId}/branches`,
      token
    );
    return response.data as StaffBranch[];
  },

  setUserBranches: async (
    userId: string,
    branchIds: string[],
    token?: string
  ): Promise<void> => {
  ...
  getPermissionResources: async (
    token?: string
  ): Promise<PermissionResourcesResponse> => {
    const apiService = ApiService.getInstance();
    const response = await apiService.get(`/api/permissions/resources`, token);
    return response.data as PermissionResourcesResponse;
  },
};
