...
export default function DealCategoryOrderingScreen() {
  ...
  const canCategoryDisplayPriority =
    !permissionsLoading &&
    canAny([{ resource: RESOURCES.CATEGORIES, action: ACTIONS.DISPLAY_PRIORITY }]);
  ...
  const loadOrderingCategories = useCallback(async () => {
    try {
      ...
      setToast({
        visible: true,
        message: t("admin.dealCategoryOrdering.loadError", {
          defaultValue: "Failed to load deal categories",
        }),
        type: "error",
      });
    } finally {
    ...
    async (type: "featured" | "list") => {
      try {
        ...
        setToast({
          ...
          message:
            type === "featured"
              ? t("admin.dealCategoryOrdering.featuredSaved", {
                  defaultValue: "Featured deal categories order updated",
                })
              : t("admin.dealCategoryOrdering.listSaved", {
                  defaultValue: "Deal categories order updated",
                }),
          type: "success",
        });
  ...
  return (
    <View style={styles.container}>
      <AnimatedHeader
        title={t("admin.pageTitles.orderingAndDisplayPriority", {
        ...
        {orderingLoading && orderingCategories.length === 0 ? (
          <View style={styles.loadingContainer}>
            ...
            <Text style={styles.loadingText}>
              {t("admin.categoryManagement.loading", { defaultValue: "Loading..." })}
            </Text>
          </View>
        ) : (
            ...
            <View style={styles.section}>
              ...
              <View style={styles.sectionActions}>
                <TouchableOpacity
                  style={[
                    styles.resetButton,
                    (!canCategoryOrdering || !listHasChanges || isSavingListOrder) && styles.resetButtonDisabled,
                  ]}
                ...
                >
                  <Text
                    style={[
                      styles.resetButtonText,
                      (!listHasChanges || isSavingListOrder) && styles.resetButtonTextDisabled,
                    ]}
                  >
                    {t("admin.categoryManagement.ordering.reset", { defaultValue: "Reset" })}
                  </Text>
                ...
                <TouchableOpacity
                  style={[
                    styles.saveButton,
                    (!canCategoryOrdering || !listHasChanges || isSavingListOrder || listItems.length === 0) &&
                      styles.saveButtonDisabled,
                  ]}
