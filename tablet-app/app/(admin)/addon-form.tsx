...
export default function AddonFormScreen() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const isSkuError = errorMessage.includes("SKU already exists");
      setToast({
        visible: true,
        message: isSkuError
          ? t("admin.addonManagement.skuAlreadyExists")
          : t("admin.addonManagement.saveAddonError"),
        type: "error",
      });
    } finally {
  ...
  return (
    ...
    >
      ...
      >
        ...
        <View style={styles.formGroup}>
          <Text style={styles.label}>
            {t("admin.addonManagement.sku")}
          </Text>
          <TextInput
            style={styles.input}
            placeholder={t("admin.addonManagement.skuPlaceholder")}
            placeholderTextColor="#6B7280"
            value={formData.sku || ""}
            onChangeText={(text) =>
              setFormData({ ...formData, sku: text.trim() || null })
            }
...
const styles = StyleSheet.create({
  ...
  typeButtonText: { color: "#111827", fontWeight: "600" },
  footer: {
    flexDirection: "row",
    gap: 12,
    padding: 16,
  ...
  typeOptionText: { fontSize: 15, color: "#111827", fontWeight: "500" },
