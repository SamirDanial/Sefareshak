...
export default function AnalyticsScreen() {
      ...
      try {
        ...
        if (result.success && result.data) {
          ...
          setSelectedBranchId((prev) => {
            // Staff with assigned branches cannot use "all" and must stay within assigned branches
            if (isBranchRestricted) {
              if (prev && prev !== "all" && filtered.some((b) => b.id === prev)) return prev;
              return filtered[0]?.id || "";
            }

            // SUPER_ADMIN/others can default to "all" when nothing is selected
            if (prev && (prev === "all" || filtered.some((b) => b.id === prev))) return prev;
            return prev || "all";
          });
        }
  ...
  useEffect(() => {
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [selectedPeriod, activeTab, getToken]);
...
const styles = StyleSheet.create({
  ...
  presetChipText: { color: "#D1D5DB", fontSize: 12, fontWeight: "500" },
  ...
  filterDropdownActive: { borderColor: "#ec4899", backgroundColor: "#171717" },
  filterDropdownText: {
    flex: 1,
    fontSize: 13,
    fontWeight: "500",
  ...
  kpiLabel: { fontSize: 12, color: "#9CA3AF", flex: 1, marginRight: 8 },
  ...
  breakdownLeft: { color: "#D1D5DB", fontSize: 14, fontWeight: "600" },
  breakdownRight: { color: "#9CA3AF", fontSize: 12 },
  tabsContainer: {
    paddingHorizontal: 16,
    paddingTop: 16,
    flexDirection: "row",
  ...
  monthCellText: { color: "#D1D5DB", fontWeight: "600" },
