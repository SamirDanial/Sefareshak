...
export const notificationApiService = {
  getUnseenNotifications: async (token?: string, organizationId?: string): Promise<NotificationItem[]> => {
  ...
  getNotifications: async (
    ...
    const response = await apiService.get(`/api/notifications?${params}`, token, {
      skipOrgHeader: Boolean(organizationId),
      headers: organizationId
        ? {
            "x-organization-id": organizationId,
