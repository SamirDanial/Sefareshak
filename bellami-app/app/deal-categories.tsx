...
const getImageUrl = (imagePath: string | null | undefined): string => {
  if (!imagePath) return "https://placehold.co/400x300?text=Deals";
...
export default function DealCategoriesScreen() {
  ...
  const { branch, customerOrganizationSlug } = useBranch();

  const [dealCategories, setDealCategories] = useState<any[]>([]);
  const [loading, setLoading] = useState(true);
  const [refreshing, setRefreshing] = useState(false);

  const visibleDealCategories = useMemo<any[]>(() => {
  ...
  const fetchDealCategories = async () => {
    try {
      ...
      const response = await apiService.getDealCategories(false, branch?.id);
      if (response?.success) {
        setDealCategories(response.data || []);
      } else {
        setDealCategories([]);
      }
    } catch (error) {
      ...
      >
        {loading ? (
          ...
          <View style={styles.grid}>
            {visibleDealCategories.map((category: any, index: number) => (
              <TouchableOpacity
                key={category.id}
                style={[
                  styles.categoryCard,
                ...
                onPress={() => {
                  router.push(`/deal-category/${category.id}` as any);
                }}
                activeOpacity={0.9}
              >
