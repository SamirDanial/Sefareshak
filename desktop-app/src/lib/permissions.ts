...
export function hasImplicitFullAccess(userType: UserType | string): boolean {
...
export function hasBranchAccess(user: RBACUser | null | undefined, branchId: string): boolean {
  if (!user) return false;
  if (user.hasFullAccess || hasImplicitFullAccess(user.userType)) return true;
