...
export default function MealFormScreen() {
  ...
  >({
    ...
    categoryId: "",
    image: undefined,
    sizes: isEditing ? [] : [{ name: "Normal", sizeType: "M" as const, price: 0, taxPercentage: 0 }],
    addOnIds: [],
    declarationIds: [],
  ...
  const removeSize = (index: number) => {
    if ((formData.sizes?.length || 0) <= 1) {
      setToast({
        visible: true,
        message: t("admin.menuManagement.atLeastOneSizeRequired") || "At least one size is required",
        type: "error",
      });
      return;
    }
  ...
  return (
    <View style={styles.container}>
      ...
      >
        ...
        <View style={styles.formGroup}>
          <Text style={styles.formLabel}>
            {t("admin.menuManagement.skuLabel") || "Article Number (SKU)"}
          </Text>
          <TextInput
            style={styles.input}
            placeholder={t("admin.menuManagement.enterSku") || "Enter article number (optional)"}
            placeholderTextColor="#6B7280"
            value={formData.sku}
        ...
        <View style={styles.formGroupRow}>
          ...
          <Switch
            ...
            trackColor={{ true: "#ec4899", false: "#f3f4f6" }}
            thumbColor="#fff"
          />
        </View>
              ...
              <View style={styles.sizeItemHeader}>
                ...
                <TouchableOpacity
                  style={[
                    ...
                    (formData.sizes?.length || 0) <= 1 && styles.removeSizeButtonDisabled,
                  ]}
                ...
                >
                  <MaterialCommunityIcons 
                    name="delete" 
                    size={16} 
                    color={(formData.sizes?.length || 0) <= 1 ? "#6B7280" : "#ef4444"} 
                  />
                </TouchableOpacity>
              </View>
              <TextInput
      ...
      >
        ...
        >
          <Pressable
            style={[styles.addSizeModalContent]}
            onPress={(e) => e.stopPropagation()}
          >
