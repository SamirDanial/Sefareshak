...
export const FloorPlanEditor: React.FC<FloorPlanEditorProps> = ({
  ...
  // Track deleted elements and new tables
  ...
  // Handlers
  const handleSelectItem = useCallback((id: string | null, type: "table" | "element" | null) => {
    setSelectedId(id);
    setSelectedType(type);
    // Open properties panel on mobile when selecting an item
    if (id && isMobile) {
  ...
  const handleDelete = useCallback(
    (id: string, type: "table" | "element") => {
      if (type === "table") {
        // Only remove from tables array, but don't track for deletion
        // (we don't delete tables from DB in floor plan editor, just unposition them)
        setTables((prev) => {
  ...
  const handleRotate = useCallback(
    (id: string, type: "table" | "element", degrees: number) => {
      if (type === "table") {
        setTables((prev) =>
          prev.map((t) =>
            t.id === id ? { ...t, rotation: (t.rotation + degrees) % 360 } : t
          )
        );
      } else {
        setElements((prev) =>
          prev.map((el) =>
            el.id === id ? { ...el, rotation: (el.rotation + degrees) % 360 } : el
          )
        );
      }
    },
  ...
  const createFloorArea = useCallback(
    (
      rect: { x: number; y: number; width: number; height: number },
      options: { color?: string; imageDataUrl?: string }
    ) => {
      const id = generateId();
      const newElement: DraggableFloorElement = {
        id,
  ...
  const handleSave = useCallback(async () => {
    ...
    try {
      ...
      // Always include tableNumber and capacity for ALL tables so existing table edits persist
      ...
      // Keep the editor state intact if save fails (especially important in production)
      // ZoneManagement already shows a toast + logs the underlying error.
    } finally {
      setSaving(false);
    }
  }, [
  ...
  return (
    <div className="fixed inset-0 z-50 bg-[#0d0d0d] flex flex-col">
      {/* Header */}
      <div className="flex items-center justify-between px-3 md:px-4 py-2 md:py-3 bg-[#151515] border-b border-[#333]">
        <div className="min-w-0 flex-1">
          <h1 className="text-white font-semibold text-base md:text-lg truncate">{t("admin.tableManagement.floorPlan.title")}</h1>
          <p className="text-gray-400 text-xs md:text-sm truncate">{t("admin.reservations.zone")}: {zoneName}</p>
        </div>
      </div>
        ...
        <div className="flex-1 relative min-h-0">
          ...
          {paintMode && pendingPaintRect && paintPalettePos && (
            <div
              ...
              className="bg-[#151515] border border-[#333] rounded-lg p-3 shadow-lg"
            >
              <div className="flex flex-wrap gap-2" style={{ maxWidth: 280 }}>
                ...
                <button
                  ...
                  className="text-xs text-gray-300 px-2 py-1 rounded bg-[#262626] border border-[#404040]"
                >
        ...
        {isMobile && !readOnly ? (
          <Sheet open={isPropertiesPanelOpen} onOpenChange={setIsPropertiesPanelOpen}>
            <SheetContent
              side="bottom"
              hideOverlay={true}
              className="bg-[#151718] border-t border-[#262626] text-white rounded-t-3xl p-0 max-h-[70vh] shadow-[0_-4px_20px_rgba(0,0,0,0.3)]"
            >
