...
export default function MealBranchAvailabilityScreen() {
  ...
  const loadAll = async (opts?: { silent?: boolean }) => {
    ...
    try {
      ...
      const firstBranchId = Array.isArray(nextBranches) && nextBranches.length > 0 ? nextBranches[0].id : "";
      if (firstBranchId) {
        applyForBranch(firstBranchId, nextRecords);
      } else {
        setSelectedBranchId("");
      }
    } finally {
    ...
    <View style={styles.container}>
      ...
      <View style={[styles.footer, { paddingBottom: Math.max(insets.bottom, 16) }]}>
        ...
        <TouchableOpacity style={styles.saveButton} onPress={handleSave} disabled={!canSave}>
          {saving ? <ActivityIndicator size="small" color="#fff" /> : <MaterialCommunityIcons name="check-circle" size={16} color="#fff" />}
