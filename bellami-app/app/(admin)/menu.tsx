...
export default function MenuManagementScreen() {
  ...
  const loadAvailabilityBranches = async () => {
    try {
      setAvailabilityLoadingBranches(true);
      ...
      try {
        ...
        if (result.success && result.data) {
          setAvailabilityBranches(result.data);
          return result.data;
        }
      } catch (err: any) {
      ...
      if (fallbackResult.success && fallbackResult.data) {
        setAvailabilityBranches(fallbackResult.data);
        return fallbackResult.data;
      }
      setAvailabilityBranches([]);
      return [];
    } catch (error) {
      ...
      setAvailabilityLoadingBranches(false);
    }
  };

  const loadAvailabilityRecords = async (mealId: string) => {
    try {
      const token = await getToken();
      const rows = await mealService.getMealBranchAvailability(mealId, token || undefined);
      const map: Record<string, MealBranchAvailability | null> = {};
      for (const r of rows || []) {
        map[r.branchId] = r;
      }
      setAvailabilityRecords(map);
      return map;
    } catch (error) {
  ...
  const applyAvailabilityForBranch = (
    branchId: string,
    records?: Record<string, MealBranchAvailability | null>
  ) => {
    const recMap = records ?? availabilityRecords;
    setAvailabilitySelectedBranchId(branchId);
    const rec = recMap[branchId];

    if (rec) {
      setAvailabilityIsAllWeek(Boolean(rec.isAvailableAllWeek));
  ...
  const openAvailabilityModal = (meal: Meal) => {
    setAvailabilityMeal(meal);
    setAvailabilityModalVisible(true);
    setAvailabilitySaving(false);
    setAvailabilityIsAllWeek(true);
    setAvailabilityWindows([]);
    setAvailabilitySelectedBranchId("");
    setAvailabilityRecords({});

    void (async () => {
      const [branches, recs] = await Promise.all([
        loadAvailabilityBranches(),
        loadAvailabilityRecords(meal.id),
      ]);

      const firstBranchId =
        Array.isArray(branches) && branches.length > 0 ? branches[0].id : "";
      if (firstBranchId) {
        applyAvailabilityForBranch(firstBranchId, recs);
      }
    })();
  };

  const getUnusedDay = (windows: AvailabilityWindowDraft[]): number | null => {
    const used = new Set(windows.map((w) => w.dayOfWeek));
    for (let d = 0; d <= 6; d++) {
  ...
  const updateWindowDay = (windowIndex: number, newDay: number) => {
    setAvailabilityWindows((prev) => {
      const existsElsewhere = prev.some(
        (w, idx) => idx !== windowIndex && w.dayOfWeek === newDay
      );
      if (existsElsewhere) return prev;
      return prev.map((w, idx) =>
        idx === windowIndex ? { ...w, dayOfWeek: newDay } : w
      );
    });
  };
  ...
  const updateWindowTime = (
    ...
    setAvailabilityWindows((prev) =>
      prev.map((w, idx) => (idx === windowIndex ? { ...w, [field]: value } : w))
    );
  };

  const handleAvailabilityTimePickerChange = (event: any, selectedDate?: Date) => {
    if (Platform.OS === "android") {
      setAvailabilityTimePickerState((prev) => ({ ...prev, visible: false }));
    }

    if (event.type === "set" && selectedDate) {
      const timeStr = formatTimeString(selectedDate);
      updateWindowTime(
        availabilityTimePickerState.windowIndex,
        availabilityTimePickerState.field,
        timeStr
      );

      if (Platform.OS === "ios") {
        setAvailabilityTimePickerState((prev) => ({ ...prev, date: selectedDate }));
      }
  ...
  const handleSaveAvailability = async () => {
    if (!availabilityMeal?.id) return;
    if (!availabilitySelectedBranchId) return;
    ...
    try {
      ...
      const payload = {
        branchId: availabilitySelectedBranchId,
        isAvailableAllWeek: availabilityIsAllWeek,
        windows: availabilityIsAllWeek
          ? []
      ...
      const saved = await mealService.upsertMealBranchAvailability(
        availabilityMeal.id,
        payload,
        token || undefined
      );
      ...
      setAvailabilityRecords((prev) => ({
        ...prev,
        [availabilitySelectedBranchId]: saved,
      }));

      setToast({
        visible: true,
        message: t("admin.menuManagement.branchAvailabilitySaved", {
          defaultValue: "Availability saved",
        }),
        type: "success",
      });
  ...
  const [availabilityBranches, setAvailabilityBranches] = useState<Array<{ id: string; name: string }>>([]);
  const [availabilityLoadingBranches, setAvailabilityLoadingBranches] = useState(false);
  const [availabilityRecords, setAvailabilityRecords] = useState<Record<string, MealBranchAvailability | null>>({});
  const [availabilitySelectedBranchId, setAvailabilitySelectedBranchId] = useState<string>("");
  ...
  const [availabilityBranchPickerVisible, setAvailabilityBranchPickerVisible] = useState(false);
  const [availabilityDayPickerState, setAvailabilityDayPickerState] = useState<{ visible: boolean; windowIndex: number }>({
    visible: false,
    windowIndex: 0,
  });
  const [availabilityTimePickerState, setAvailabilityTimePickerState] = useState<{
    visible: boolean;
    date: Date;
    windowIndex: number;
    field: "startTime" | "endTime";
  ...
  if (showCategoriesView) {
    return (
            ...
            <View style={styles.categoriesHeader}>
              <View style={styles.categoriesHeaderLeft}>
                ...
                {canReorderFeaturedMeals && (
                  <TouchableOpacity
                    onPress={() =>
                      router.push("/(admin)/featured-meals-ordering" as any)
                    }
      ...
      >
        ...
        >
          <Pressable
            style={[styles.bottomSheetContent, { maxHeight: "90%" }]}
            onPress={(e) => e.stopPropagation()}
          >
            <View style={styles.bottomSheetHeader}>
              <Text style={styles.bottomSheetTitle}>
                {t("admin.menuManagement.branchAvailability")}
              </Text>
              <TouchableOpacity
                onPress={() => {
                  if (availabilitySaving) return;
            ...
            <ScrollView style={styles.bottomSheetBody} keyboardShouldPersistTaps="handled">
              {availabilityMeal?.name ? (
                <Text style={styles.detailsLabel}>{availabilityMeal.name}</Text>
              ) : null}

              <View style={{ gap: 10, marginTop: 12 }}>
                <View style={styles.detailsRow}>
                  <Text style={styles.detailsLabel}>{t("admin.menuManagement.branch")}</Text>
                  <TouchableOpacity
                    style={styles.availabilitySelect}
                    onPress={() => {
                      if (availabilityLoadingBranches) return;
                      setAvailabilityBranchPickerVisible(true);
                    }}
                  >
                    <Text style={styles.availabilitySelectText}>
                      {availabilitySelectedBranchId
                        ? availabilityBranches.find((b) => b.id === availabilitySelectedBranchId)?.name ||
                ...
                <View style={styles.availabilityToggleRow}>
                  ...
                  <Switch
                    value={availabilityIsAllWeek}
                    onValueChange={(v) => setAvailabilityIsAllWeek(v)}
                    trackColor={{ false: "#374151", true: "#ec4899" }}
                    thumbColor={availabilityIsAllWeek ? "#fff" : "#9CA3AF"}
                  />
                </View>

                {!availabilityIsAllWeek && (
                  <View style={{ gap: 10 }}>
                    <View style={styles.availabilityWindowsHeader}>
                      <Text style={styles.detailsLabel}>
                        {t("admin.menuManagement.branchAvailabilityWindows")}
                      </Text>
                      <TouchableOpacity
                        style={styles.availabilityAddWindowButton}
                        onPress={() => {
                          const nextDay = getUnusedDay(availabilityWindows);
                        ...
                        disabled={getUnusedDay(availabilityWindows) === null}
                      >
                        <MaterialCommunityIcons name="plus" size={14} color="#ec4899" />
                        <Text style={styles.availabilityAddWindowText}>
                          {t("admin.menuManagement.addWindow", { defaultValue: "Add" })}
                        </Text>
                      </TouchableOpacity>
                    </View>

                    {availabilityWindows.length === 0 ? (
                      <Text style={styles.inputHint}>{t("admin.menuManagement.noWindows")}</Text>
                    ) : (
                      availabilityWindows.map((w, idx) => {
                        ...
                        return (
                          <View key={`${w.dayOfWeek}-${idx}`} style={styles.availabilityWindowCard}>
                            <View style={styles.availabilityWindowHeader}>
                              <TouchableOpacity
                                style={styles.availabilityDaySelect}
                                onPress={() =>
                                  setAvailabilityDayPickerState({ visible: true, windowIndex: idx })
                                }
                              >
                                <Text style={styles.availabilityDayText}>
                                  {t(`home.servingHours.${DAY_KEYS[w.dayOfWeek]}`)}
                                </Text>
                                <MaterialCommunityIcons
                                  name="chevron-down"
                                  size={16}
                            ...
                            <View style={styles.availabilityTimeRow}>
                              <TouchableOpacity
                                ...
                                onPress={() => {
                                  setAvailabilityTimePickerState({
                                    visible: true,
                              ...
                              >
                                <Text style={styles.availabilityTimeLabel}>
                                  {t("admin.menuManagement.branchAvailabilityStart")}
                                </Text>
                                <Text style={styles.availabilityTimeValue}>{w.startTime}</Text>
                              </TouchableOpacity>
                              ...
                              >
                                ...
                                <Text style={styles.availabilityTimeValue}>{w.endTime}</Text>
                              </TouchableOpacity>
                            </View>

                            {availabilityDayPickerState.visible &&
                            availabilityDayPickerState.windowIndex === idx ? (
                              <Modal
                                visible={availabilityDayPickerState.visible}
                                transparent
                                animationType="fade"
                                onRequestClose={() =>
                                  setAvailabilityDayPickerState((prev) => ({ ...prev, visible: false }))
                                }
                              >
                                <Pressable
            ...
            <View style={styles.availabilityFooter}>
              ...
              >
                <Text style={styles.availabilityCancelText}>{t("common.cancel")}</Text>
              </TouchableOpacity>
              <TouchableOpacity
                style={styles.availabilitySaveButton}
                onPress={handleSaveAvailability}
                disabled={availabilitySaving || !availabilitySelectedBranchId}
              >
                {availabilitySaving ? (
                ...
                <Text style={styles.availabilitySaveText}>
                  {availabilitySaving ? t("admin.menuManagement.saving") : t("common.save")}
                </Text>
              </TouchableOpacity>
            </View>
          </Pressable>
      ...
      <Modal
        visible={availabilityBranchPickerVisible}
        transparent
        animationType="slide"
        onRequestClose={() => setAvailabilityBranchPickerVisible(false)}
      >
        <Pressable
          style={styles.bottomSheetOverlay}
          onPress={() => setAvailabilityBranchPickerVisible(false)}
        >
          <Pressable
          ...
          >
            <View style={styles.bottomSheetHeader}>
              ...
              <TouchableOpacity onPress={() => setAvailabilityBranchPickerVisible(false)}>
                <MaterialCommunityIcons name="close" size={20} color="#9CA3AF" />
              </TouchableOpacity>
            </View>
            <ScrollView style={styles.bottomSheetBody}>
              {availabilityLoadingBranches ? (
                <View style={{ paddingVertical: 16, alignItems: "center" }}>
                  <ActivityIndicator size="small" color="#ec4899" />
                </View>
              ) : (
                availabilityBranches.map((b) => {
                  const selected = b.id === availabilitySelectedBranchId;
                  return (
                    <TouchableOpacity
                      key={b.id}
                      style={[styles.bottomSheetOption, selected && styles.bottomSheetOptionActive]}
                      onPress={() => {
                        applyAvailabilityForBranch(b.id);
                        setAvailabilityBranchPickerVisible(false);
                      }}
                    >
                      <Text
                        style={[
                          styles.bottomSheetOptionText,
                          selected && styles.bottomSheetOptionTextActive,
                        ]}
                      >
                        {b.name}
                      </Text>
      ...
      {availabilityTimePickerState.visible && (
        <>
          {Platform.OS === "ios" ? (
            <Modal
              visible={availabilityTimePickerState.visible}
              transparent
              animationType="slide"
              onRequestClose={() =>
                setAvailabilityTimePickerState((prev) => ({ ...prev, visible: false }))
                ...
                <View style={styles.timePickerModalContent}>
                  <View style={styles.modalHeader}>
                    <Text style={styles.modalTitle}>
                      {availabilityTimePickerState.field === "startTime"
                        ? t("admin.menuManagement.branchAvailabilityStart")
                        : t("admin.menuManagement.branchAvailabilityEnd")}
                    </Text>
                    <TouchableOpacity
                  ...
                  <View style={styles.timePickerContainer}>
                    <DateTimePicker
                      value={availabilityTimePickerState.date}
                      mode="time"
                      is24Hour={false}
                      display="spinner"
                      onChange={handleAvailabilityTimePickerChange}
                      textColor="#fff"
                      themeVariant="dark"
                  ...
                  <View style={styles.timePickerActions}>
                    ...
                    <TouchableOpacity
                      ...
                      onPress={() => {
                        const timeStr = formatTimeString(availabilityTimePickerState.date);
                        updateWindowTime(
                          availabilityTimePickerState.windowIndex,
                          availabilityTimePickerState.field,
                          timeStr
              ...
              <View style={styles.sheetContent}>
                ...
                {canUpdateMeal && (
                  <TouchableOpacity
                    style={styles.sheetItem}
                    onPress={() => {
                      const meal = actionsMeal as Meal | null;
                      setActionsModalVisible(false);
                      setShowActionsMenu(null);
                      setActionsMeal(null);
                          ...
                          .map((bp) => {
                            ...
                            return (
                              <View key={bp.id || bp.branchId} style={styles.detailsRow}>
                                <Text style={styles.detailsLabel}>{bpName}</Text>
                                <Text style={styles.detailsValue}>
                                  {bpPrice} · {bpTax}
                                </Text>
                              </View>
...
const styles = StyleSheet.create({
  ...
  availabilityWindowsHeader: {
    flexDirection: "row",
    alignItems: "center",
    justifyContent: "space-between",
  },
  availabilityAddWindowButton: {
    flexDirection: "row",
    alignItems: "center",
    gap: 6,
    paddingHorizontal: 10,
