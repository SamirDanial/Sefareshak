...
const StaffManagement: React.FC = () => {
  ...
  const getOrgRoleBadge = (r?: OrgRole | null) => {
    ...
    if (r === "ORG_OWNER")
      return {
        label: t("admin.staffManagement.orgOwnerBadge", { defaultValue: "ORG OWNER" }),
        className:
          "bg-amber-100 text-amber-800 dark:bg-amber-900/30 dark:text-amber-300",
      };
    if (r === "ORG_ADMIN")
      return {
        label: t("admin.staffManagement.orgAdminBadge", { defaultValue: "ORG ADMIN" }),
        className:
          "bg-blue-100 text-blue-800 dark:bg-blue-900/30 dark:text-blue-300",
      };
    return {
      label: t("admin.staffManagement.orgStaffBadge", { defaultValue: "ORG STAFF" }),
      className:
  ...
  useEffect(() => {
    const applyOrgId = (next: string | null | undefined) => {
      setSelectedOrganizationId(String(next || "").trim());
    };
  ...
  return (
    <div className="space-y-6 p-6">
      <PageHeader
        title={t("admin.staffManagement.title", { defaultValue: "Staff Management" })}
        description={t("admin.staffManagement.description", {
        ...
        <DialogContent className="bg-card border-border text-foreground max-w-2xl">
          ...
          <div className="space-y-5">
            <div className="space-y-2">
              ...
              {(userType === "SUPER_ADMIN" || orgRole === "ORG_OWNER") &&
                activeUser &&
                isLastOrgOwner(activeUser) && (
                            ...
                            {branches.map((b) => (
                              <label key={b.id} className="flex items-center gap-2 text-sm">
                                <Checkbox
                                  ...
                                  onCheckedChange={() =>
                                    setEditBranchIds((prev) => toggleInArray(prev, b.id))
                                  }
                                />
                        ...
                        <div className="space-y-2">
                            ...
                            {roles.map((r) => (
                              <label key={r.id} className="flex items-center gap-2 text-sm">
                                <Checkbox
                                  ...
                                  onCheckedChange={() =>
                                    setEditRoleIds((prev) => toggleInArray(prev, r.id))
                                  }
                                />
                          ...
                          <div className="text-xs text-muted-foreground">
                            {t("admin.staffManagement.rolesAssignedGloballyNote", {
                              defaultValue:
                                "Roles are currently assigned globally (not per-branch) in this UI.",
                            })}
                          </div>
                        </div>
                      )}
      ...
      <Dialog open={removeDialogOpen} onOpenChange={setRemoveDialogOpen}>
        <DialogContent className="bg-card border-border text-foreground max-w-md">
          <DialogHeader>
            <DialogTitle>{t("admin.staffManagement.removeFromOrg", { defaultValue: "Remove" })}</DialogTitle>
          </DialogHeader>
